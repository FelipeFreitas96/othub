...
export class Connection {
    ...
    connecting: boolean
    ws: WebSocket | null
    wsReady: Promise<WebSocket> | null
    recvBuffer: Uint8Array
    checksumEnabled: boolean
    xteaEnabled: boolean
    xteaKey: number[] | null
    sequencedPacketsEnabled: boolean = false
    ...
    pushData(chunk: Uint8Array) {
        ...
        while (this.recvBuffer.length >= 2) {
            ...
            if (payload) {
                if (typeof window !== 'undefined' && (window as any).__otDebugBridge) {
    ...
    decodePacket(packet: Uint8Array): Uint8Array | null {
        let data = packet
    ...
    buildPacket(payload: Uint8Array): Uint8Array {
        let body = payload
