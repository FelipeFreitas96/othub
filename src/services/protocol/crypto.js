...
export function writeU16LE(buffer, offset, value) {
    buffer[offset] = value & 0xff
    buffer[offset + 1] = (value >> 8) & 0xff
}

export function writeU32LE(buffer, offset, value) {
    buffer[offset] = value & 0xff
    buffer[offset + 1] = (value >> 8) & 0xff
...
export function readU16LE(buffer, offset = 0) {
    return buffer[offset] | (buffer[offset + 1] << 8)
}

export function readU32LE(buffer, offset = 0) {
    return (
        buffer[offset] |
        (buffer[offset + 1] << 8) |
        (buffer[offset + 2] << 16) |
...
export function xteaEncrypt(data, key) {
...
export function getRsaKeySize(rsaN) {
...
export function rsaEncrypt(block, rsaN, rsaE = '65537') {
    const modulus = BigInt(rsaN)
