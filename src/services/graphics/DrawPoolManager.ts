...
import { DrawPool, DrawPoolType, DrawOrder, type DrawObject, type PoolState } from './DrawPool'
...
export class DrawPoolManager {
  ...
  /** OTC: draw() – iterate all pool types and draw each */
  draw(): void {
    const viewportSize = (typeof window !== 'undefined' && window.document?.body)
      ? { width: window.innerWidth, height: window.innerHeight }
      : this.m_size
    if (this.m_size.width !== viewportSize.width || this.m_size.height !== viewportSize.height) {
  ...
  drawObject(pool: DrawPool, obj: DrawObject): void {
    if (obj.action) {
      ...
      g_painter.drawCoords(rects.length ? rects : [obj.coords], obj.state ?? undefined)
    }
  }
  ...
  /** OTC: addTexturedRect */
  addTexturedRect(dest: { x?: number; y?: number; width?: number; height?: number }, texture: unknown, src?: { x?: number; y?: number; width?: number; height?: number }, color?: unknown): void {
    const d = dest ?? {}
    const s = src ?? {}
    ...
    this.getCurrentPool()?.add(color as any, texture, { type: 0, dest: d as any, src: s as any })
  }
  ...
  preDraw(
    ...
    if (pool.hasFrameBuffer()) {
      this.addAction(() => {
        ;(pool as any).m_framebuffer?.prepare?.(dest, src, colorClear)
      })
    }
    pool.release()
    resetSelectedPool()
  ...
  drawObjects(pool: DrawPool): void {
    ...
    if (!shouldRepaint && hasFramebuffer) return
    if (hasFramebuffer) (pool as any).m_framebuffer?.bind?.()
    if (shouldRepaint) {
    ...
    const target = (pool as any).scene != null
      ? { scene: (pool as any).scene, tileGroups: (pool as any).tileGroups ?? [], w: (pool as any).w ?? 0, h: (pool as any).h ?? 0 }
      : null
    if (target) g_painter.setDrawTarget(target)
    for (const obj of list) {
      this.drawObject(pool, obj)
    }
    if (target) g_painter.setDrawTarget(null)
    if (hasFramebuffer) (pool as any).m_framebuffer?.release?.()
  ...
  drawPool(type: DrawPoolType): void {
    ...
    if (pool.hasFrameBuffer()) {
      ...
      if ((pool as any).m_beforeDraw) (pool as any).m_beforeDraw()
      ;(pool as any).m_framebuffer?.draw?.()
      if ((pool as any).m_afterDraw) (pool as any).m_afterDraw()
    }
  }
  ...
  get thingsRef(): { current: { types: unknown; sprites: unknown } } | null {
    return this.getCurrentPool()?.thingsRef ?? null
  }
  get w(): number {
  ...
  addTexturedRect(rect: any): void {
    this.getCurrentPool()?.addTexturedRect(rect)
  }
  ...
  beginCreatureInfo(p: { x: number; y: number }, mapRect?: any): void {
    this.getCurrentPool()?.beginCreatureInfo(p, mapRect)
  }
  endCreatureInfo(): void {
    this.getCurrentPool()?.endCreatureInfo()
  }
  calcFirstVisibleFloor(types: unknown): number {
    return this.getCurrentPool()?.calcFirstVisibleFloor(types) ?? 7
  }
  calcLastVisibleFloor(firstFloor: number): number {
    return this.getCurrentPool()?.calcLastVisibleFloor(firstFloor) ?? firstFloor
  }
  ...
  shaderNeedFramebuffer(): boolean {
    return this.getCurrentPool()?.shaderNeedFramebuffer() ?? false
  }
