...
export class ThingType {
  ...
  m_elevation: number
  m_opacity: number
  m_displacement: { x: number, y: number }
  m_size: { width: number, height: number }
  m_attribs: Map<number, any>
  ...
  hasAttr(attr: number) { return this.m_attribs.has(attr) }
   ...
   */
  getSpriteIndex(w: number, h: number, l: number, x: number, y: number, z: number, a: number) {
   ...
   */
  unserialize(clientId: number, category: ThingCategory, fin: any) {
    this.m_null = false
    this.m_id = clientId
  ...
  _readAnimator(fin: any, phases: number) {
