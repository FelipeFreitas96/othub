...
import { ThingCategory, ThingType } from './thingType'

export class ThingTypeManager {
  m_nullThingType: ThingType
  m_datLoaded: boolean
  m_xmlLoaded: boolean
  m_otbLoaded: boolean
  m_datSignature: number
  m_contentRevision: number
  m_otbMinorVersion: number
  m_otbMajorVersion: number
  m_thingTypes: ThingType[][]
  ...
  isValidDatId(id: number, category: number) { return id >= 1 && id < (this.m_thingTypes[category]?.length ?? 0) }

  getThingType(id: number, category: number) {
  ...
  getItem(clientId: number) { return this.getThingType(clientId, ThingCategory.Item) }
  getCreature(clientId: number) { return this.getThingType(clientId, ThingCategory.Creature) }
  getEffect(clientId: number) { return this.getThingType(clientId, ThingCategory.Effect) }
  getMissile(clientId: number) { return this.getThingType(clientId, ThingCategory.Missile) }
  ...
  loadDat(arrayBuffer: ArrayBuffer) {
