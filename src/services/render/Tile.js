...
import { Item } from '../things/Item.js'
...
export class Tile {
  ...
  /**
   * OTC: drawThing(thing, dest, flags, drawElevation, lightView)
   * newDest = dest - drawElevation * scaleFactor; thing->draw(newDest); updateElevation(thing, drawElevation);
   * (OTC não tem zOff; passamos 0.)
   */
  _drawThing(thing, pipeline, drawElevationPx, steps, state) {
    const elev = drawElevationPx
    steps.push(() => { thing.draw(pipeline, this.x, this.y, elev, 0, this.z) })
    if (state) this.updateElevation(thing, state)
  }

  /**
   * OTC: Tile::drawCreature(dest, flags, forceDraw, drawElevation, lightView)
   * Non-walking creatures first (forward), then m_walkingCreatures with offset.
   */
   ...
   * drawElevation = 0, then effects, then onTop items.
   */
  drawTop(pipeline, drawFlags, forceDraw, state, steps) {
    ...
    if (drawFlags & DrawEffects && this.m_effects.length > 0) {
      for (const effect of this.m_effects) {
        steps.push(() => { effect.draw?.(pipeline, this.x, this.y, 0, 0, this.z) })
      }
    }
    if (drawFlags & DrawOnTop && this.hasTopItem()) {
      for (const thing of this.m_things) {
        if (!thing.isOnTop?.()) continue
        this._drawThing(thing, pipeline, 0, steps, state)
      }
    }
  }
